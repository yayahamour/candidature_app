...
class Candidacy(db.Model):
    ...
    def get_all_in_list_with_user_name(cls):
        candidacy_list = []
        for candidacy in cls.query.join(Users).with_entities(Users.first_name, cls.entreprise, cls.contact_full_name, cls.contact_email, cls.contact_mobilephone, cls.date, cls.modified_date, cls.relance, cls.status).all():
...
def init_db():
    ...
    Candidacy(user_id=1, entreprise="facebook", contact_full_name="mz",
